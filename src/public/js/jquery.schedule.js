...
;(function ($, window, document, undefined) {
  ...
  $.extend(Plugin.prototype, {
    ...
    /**
     * Calculate height related to max hours per day
     */

    calculateHeight: function() {
      const normalHeight = 960;
      return Math.floor((normalHeight / 24) * (24 - this.periodMin));
    },
    ...
    create: function () {
      ...
      for (var k = 0; k < 7; k++) {
        var dayNb = k + this.dayFirstNb;
        $('<div class="jqs-grid-day">' + this.settings.days[k] + " " + dayNb + dayRemove + dayDuplicate + '</div>').
    ...
    periodFormat: function (position) {
      ...
      position += this.periodMin * this.periodInterval;
    ...
    positionFormat: function (time) {
      ...
      var position = 0;
      position += (hour - this.periodMin) * this.periodInterval;
